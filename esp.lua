...
if not Drawing or not Drawing.new or not Drawing.Fonts then
	loadstring(game.HttpGet(game, "https://raw.githubusercontent.com/Stratxgy/esp-aimbot/refs/heads/main/drawing.lua"))()
...
local ConfigLibrary = loadstring(game.HttpGet(game, "https://raw.githubusercontent.com/Stratxgy/esp-aimbot/refs/heads/main/configlib.lua"))()
...
if not Degrade then
	...
	local DrawQuad = loadstring(game.HttpGet(game, "https://raw.githubusercontent.com/Stratxgy/esp-aimbot/refs/heads/main/QuadRender.lua"))() -- Quad Drawing Object
...
getgenv().ESP = {
	DeveloperSettings = {
		Path = "ESP/ESP/Configuration.cfg",
...
local CoreFunctions = {
	...
	GenerateHash = function(Bits)
		...
		for _ = 1, Bits do
			Result ..= ("ESP")[mathrandom(1, 2) == 1 and "upper" or "lower"](stringchar(mathrandom(97, 122)))
		end
...
local UtilityFunctions = {
	...
	WrapObject = function(self, Object, PseudoName, Allowed, RenderDistance)
		assert(self, "ESP > UtilityFunctions.WrapObject - Internal error, unassigned parameter \"self\".")
		...
		if not Entry.IsAPlayer then
			if not pcall(function()
				...
				warn("ESP > UtilityFunctions.WrapObject - Attempted to wrap object of an unsupported class type: \""..(__index(Entry.Object, "ClassName") or "N / A").."\"")
...
Environment.UnwrapAll = function(self) -- METHOD | (<void>) => <void>
	assert(self, "ESP.UnwrapAll: Missing parameter #1 \"self\" <table>.")
...
Environment.Restart = function(self) -- METHOD | (<void>) => <void>
	assert(self, "ESP.Restart: Missing parameter #1 \"self\" <table>.")
...
Environment.Exit = function(self) -- METHOD | (<void>) => <void>
	assert(self, "ESP.Exit: Missing parameter #1 \"self\" <table>.")
	...
	if self:UnwrapAll() then
		...
		for Index, _ in next, Environment do
			getgenv().ESP[Index] = nil
		end

		LoadESP = nil; Recursive = nil; Loaded = false

		getgenv().DeveloperESP = nil
	end
end
...
Environment.UpdateConfiguration = function(DeveloperSettings, Settings, Properties) -- (<table> DeveloperSettings, <table> Settings, <table> Properties) => <table> New Environment
	assert(DeveloperSettings, "ESP.UpdateConfiguration: Missing parameter #1 \"DeveloperSettings\" <table>.")
	assert(Settings, "ESP.UpdateConfiguration: Missing parameter #2 \"Settings\" <table>.")
	assert(Properties, "ESP.UpdateConfiguration: Missing parameter #3 \"Properties\" <table>.")

	getgenv().ESP.DeveloperSettings = DeveloperSettings
	getgenv().ESP.Settings = Settings
	getgenv().ESP.Properties = Properties

	Environment = getgenv().ESP

	return Environment
end

Environment.LoadConfiguration = function(self) -- METHOD | (<void>) => <void>
	assert(self, "ESP.LoadConfiguration: Missing parameter #1 \"self\" <table>.")
...
Environment.SaveConfiguration = function(self) -- METHOD | (<void>) => <void>
	assert(self, "ESP.SaveConfiguration: Missing parameter #1 \"self\" <table>.")
